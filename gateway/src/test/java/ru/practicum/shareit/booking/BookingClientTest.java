...
class BookingClientTest {
    ...
    void createBooking_ShouldCallPost() {
        ...
        verify(restTemplate).exchange(
                eq(""),  // BookingClient использует относительный путь ""
    ...
    void approveBooking_ShouldCallPatchWithParameters() {
        ...
        assertEquals(expectedResponse, response);
        verify(restTemplate).exchange(
                eq("/1?approved={approved}"),  // Относительный путь с параметром
    ...
    void cancelBooking_ShouldCallPatch() {
        ...
        assertEquals(expectedResponse, response);
        verify(restTemplate).exchange(
                eq("/1/cancel"),  // Относительный путь
                eq(org.springframework.http.HttpMethod.PATCH),
                any(),
                eq(Object.class)
        );
    ...
    void getUserBookings_ShouldCallGetWithParameters() {
        ...
        assertEquals(expectedResponse, response);
        verify(restTemplate).exchange(
                eq("?state={state}&from={from}&size={size}"),  // Относительный путь с параметрами
                eq(org.springframework.http.HttpMethod.GET),
                any(),
                eq(Object.class),
    ...
    void getOwnerBookings_ShouldCallGetWithParameters() {
        ...
        assertEquals(expectedResponse, response);
        verify(restTemplate).exchange(
                eq("/owner?state={state}&from={from}&size={size}"),  // Относительный путь с параметрами
                eq(org.springframework.http.HttpMethod.GET),
                any(),
                eq(Object.class),
                eq(java.util.Map.of("state", "ALL", "from", 0, "size", 10))
