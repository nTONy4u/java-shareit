...
class UserControllerTest {
    ...
    @Test
    void updateUser_ValidUpdate_ShouldReturnOk() throws Exception {
        UserDto userDto = new UserDto(null, "Updated Name", "updated@example.com");
q        when(userClient.updateUser(ArgumentMatchers.eq(1L), ArgumentMatchers.any(UserDto.class)))
                .thenReturn(ResponseEntity.ok().build());
    ...
    @Test
    void updateUser_BlankName_ShouldReturnBadRequest() throws Exception {
        UserUpdateDto userUpdateDto = new UserUpdateDto("", "valid@example.com");

        mockMvc.perform(patch("/users/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(userUpdateDto)))
                .andExpect(status().isBadRequest());  // Ожидает 400
    }

    @Test
    void updateUser_BlankEmail_ShouldReturnBadRequest() throws Exception {
    ...
    void updateUser_InvalidEmail_ShouldReturnBadRequest() throws Exception {
        UserUpdateDto userUpdateDto = new UserUpdateDto("Updated Name", "invalid-email");
